...
export default function LastMonthReport() {
  ...
  startOfMonth.setMonth(startOfMonth.getMonth() - 1); // go to last month
  startOfMonth.setDate(1); // first day
  startOfMonth.setHours(0, 0, 0, 0);

  // Get last month end date
  const endOfMonth = new Date();
  endOfMonth.setDate(0); // this automatically goes to last day of previous month
  endOfMonth.setHours(23, 59, 59, 999);

  const lastMonthSales = salesData?.filter((sales) => {
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Last Month Sales Report - ${formattedMonth}`, 14, 25);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Last Monthly Sales");

    //Download Excel file
    XLSX.writeFile(
      workbook,
      `last_monthly_sales_report_${formattedMonth}.xlsx`,
    );
  };
